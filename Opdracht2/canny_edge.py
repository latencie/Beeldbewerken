...
#This file outputs the edges of the cameraman image using the canny edge detection method
  ...
  for i in range(0, width - 1):
    for j in range(0, height - 1):
      ...
      if(first_check > t):
        ...
        #The second check uses the zero crossings method to check for an edge
        if(zero_crossings(F, s, Fx, Fy, Fxx, Fyy, Fxy, i, j)):
          new_img[i, j] = first_check

  return new_img

#This function does the zero crossing check of a point and its 3x3 neighbourhood
def zero_crossings(F, s, Fx, Fy, Fxx, Fyy, Fxy, x, y):
  ...
  for i in range(0, 4):
    ...
    #Calculate the fww of the first pixel
...
def g2(x, s):
  return  ((-(s**2 - x**2) / s**5 * sqrt(2.0 * pi)) * exp(-(x**2 / (2.0 * s**2))))
